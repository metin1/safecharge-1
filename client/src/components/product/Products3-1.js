...
let cardNumber = '2221008123677736';
let cardHolderName = 'CL-BRW2';
let expirationMonth = '12';
let expirationYear = '25';
let CVV = '217';
let methodNotificationUrl = 'http://localhost:5000//notification';
// let methodNotificationUrl = 'https://3dsecuresafecharge.000webhostapp.com/3Dv2/notificationUrl.php';
...
class Products extends Component {
  ...
  initialPayment = async () => {
    ...
    axios(config)
      .then(function (response) {
        ...
        // console.log((response.data && response.data.paymentOption && response.data.paymentOption.card && response.data.paymentOption.card.threeD && response.data.paymentOption.card.threeD.methodPayload))
        // console.log((response.data && response.data.paymentOption && response.data.paymentOption.card && response.data.paymentOption.card.threeD && response.data.paymentOption.card.threeD.methodUrl))
  ...
  payment = (response) => {
    //Payment
    let currentComponent = this;
    relatedTransactionId = (response && response.data && response.data.transactionId) 
    var data = JSON.stringify({
      sessionToken,
      merchantId,
      merchantSiteId,
      ...
      relatedTransactionId:
        (response && response.data && response.data.transactionId) || '',
      paymentOption: {
        card: {
          cardNumber,
          cardHolderName,
          ...
          threeD: {
            ...
            version: '2.1.0',
            // notificationURL:methodNotificationUrl,
            notificationURL:
    ...
    axios(config)
      .then(function (response) {
        ...
        console.log('transactionStatus:  ', response.data.transactionStatus);
        if (response.data.transactionStatus === 'REDIRECT') {
          console.log('transactionStatus:  ', response.data.transactionStatus);
          currentComponent.Challenge(
            response.data &&
              ...
              response.data.paymentOption.card &&
              response.data.paymentOption.card.threeD &&
              response.data.paymentOption.card.threeD.acsUrl,
            response.data &&
              response.data.paymentOption &&
              response.data.paymentOption.card &&
              response.data.paymentOption.card.threeD &&
              response.data.paymentOption.card.threeD.cReq
          );
        }
        if (response.data.transactionStatus === 'APPROVED') {
          Swal.fire(
            'Success!',
            `Your transaction ${response.data.transactionStatus}`,
            'success'
          );
        }
      })
  ...
  liabilityShift = (response) => {
    var data = JSON.stringify({
      ...
      amount: '10',
      checksum:
        '25945f39ab91003a07fbf4b9a9298e8511b63de59e234d790001348bcc22dd98',
      relatedTransactionId: relatedTransactionId || (response && response.data && response.data.transactionId) || '',
      currency: 'EUR',
      amount: '150',
    ...
    console.log('liabilityShift Request:  ', {
      sessionToken,
      merchantId,
      merchantSiteId,
      clientRequestId,
    ...
    axios(config)
      .then(function (response) {
        console.log( 'liabilityShift: ', response.data);
      })
      .catch(function (error) {
        console.log(error);
      });
  ...
  Challenge = (acsUrl, cReq) => {
    let currentComponent = this;
    console.log(`OUTPUT: Products -> Challenge -> cReq decode`, Buffer.from(cReq, 'base64').toString('ascii'));
    console.log(`OUTPUT: Products -> Challenge -> acsUrl`, acsUrl);
    var data = '';
    var config = {
      method: 'get',
      url: `https://3dsecuresafecharge.000webhostapp.com/3Dv2/showUrl.php?acsUrl=${acsUrl}&creq=${cReq}`,
      headers: {
    ...
    axios(config)
      .then(function (response) {
        ...
        currentComponent.liabilityShift(response);
      })
      .catch(function (error) {
        console.log(error);
        currentComponent.liabilityShift();
      });
  };
